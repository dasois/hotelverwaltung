...
public class CreateRoomFrame extends AbstractFrame {
	...
	protected void createWidget() {
		...
		roomPrice.setFont(header.getFont().deriveFont(Font.BOLD + Font.ITALIC,
				20));

		doubleRoom = new JLabel("Doppelzimmer? ");
		doubleRoom.setFont(header.getFont().deriveFont(Font.BOLD + Font.ITALIC,
				20));
		...
		create.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (e.getActionCommand() == "Create")
					...
					try {
						int roomId = controller.create(
								Double.parseDouble(price.getText()),
								doubleRoomCheck.isSelected());
						mf.addProtocolLine("Zimmer: " + roomId
								+ " wurde mit dem Preis: "
								+ Double.parseDouble(price.getText())
