		...
		if (e.getActionCommand().equals("action")) {
			...
			if (khd == "Kunden") {
				if (action == "Erstellen") {
					...
					try {
						dcf = new DeleteFrame<Customer>(gui, "Kunde Entfernen",
								new JList<Customer>(
										new CustomerControlImp().getAll()));
						dcf.init();
						new JDialog(dcf);
				...
				if (action == "Erstellen") {
					...
					gui.setEnabled(false);
					SelectTimeIntervallRoomFrame rf = new SelectTimeIntervallRoomFrame(
							gui);
					rf.init();
					new JDialog(rf);
				} else {
					...
					// TODO sinnvolle Exception
					try {
						drf = new DeleteFrame<Room>(gui, "Zimmer Entfernen",
								new JList<Room>(new RoomControlImp().getAll()));
						drf.init();
						new JDialog(drf);
					} catch (SQLException e1) {
						e1.printStackTrace();
				...
				if (action == "Erstellen") {
					...
					// TODO sinnvolle Exception
					try {
						dsf = new DeleteFrame<Service>(gui,
								"Service Entfernen", new JList<Service>(
										new ServiceControlImp().getAll()));
						dsf.init();
						new JDialog(dsf);
					} catch (SQLException e1) {
						e1.printStackTrace();
