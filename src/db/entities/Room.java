...
public class Room implements DBRoom {
	...
	public ResultSet getFree(Date startDate, Date endDate) throws SQLException {
		return DBIface
				.executeQuery("SELECT * from Room r "
						+ "WHERE NOT EXISTS ( SELECT * FROM Booking_Room WHERE (Date between \""
	...
	public int create() throws SQLException {
		ResultSet rs = DBIface.executeQuery("Insert into Room values ("
				+ this.getRid() + "," + this.getPrice() + ","
				+ this.isDoubleRoom() + ")");
		if (rs.next()) {
	...
	public boolean update() throws SQLException {
		DBIface.executeQuery("Update Room set " + "Price = " + this.getPrice()
				+ ",isDoubleRoom = " + this.isDoubleRoom() + " Where RID = "
				+ this.getRid());
		ResultSet rs = DBIface
				.executeQuery("SELECT COUNT(*) from Room where rid = "
						+ this.getRid());
		rs.next();
		if (rs.getInt(1) == 1) {
			return true;
