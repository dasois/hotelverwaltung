...
public class Service implements DBService {
	...
	public int create() throws SQLException {
		ResultSet rs = DBIface.executeQuery("Insert into Service values ("
				+ this.getSid() + ", \"" + this.getType() + "\","
				+ this.getPrice() + ")");
		if (rs.next()) {
	...
	public boolean update() throws SQLException {
		DBIface.executeQuery("Update Service set " + "Type = \""
				+ this.getType() + "\"" + ",Price = " + this.getPrice()
				+ " Where SID = " + this.getSid());
		ResultSet rs = DBIface
				.executeQuery("SELECT COUNT(*) from Service where sid = "
						+ this.getSid());
		rs.next();
		if (rs.getInt(1) == 1) {
			return true;
